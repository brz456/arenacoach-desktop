...
class AuthUI {
    ...
    setupElements() {
        this.loginBtn = document.getElementById('auth-action-btn');
        // Cache login button sub-elements for safe DOM manipulation
        this.loginBtnLabel = this.loginBtn?.querySelector('.filter-label');
        this.loginBtnIconPath = this.loginBtn?.querySelector('svg path');
    }

    setupEvents() {
        this.loginBtn?.addEventListener('click', async () => {
            // Double-check authentication state with IPC before proceeding
            try {
                const currentAuthState = await window.arenaCoach.auth.isAuthenticated();
                
                if (currentAuthState !== this.isAuthenticated) {
                    this.isAuthenticated = currentAuthState;
                    
                    // Update UI to match correct state
                    if (currentAuthState) {
                        const currentUser = await window.arenaCoach.auth.getCurrentUser();
                ...
                console.error('[AuthUI] Failed to verify auth state:', error);
            }
            
            if (this.isAuthenticated) {
                this.handleLogout();
            } else {
                this.handleBattleNetLogin();
            }
        });
    }
    ...
    setupAuthListeners() {
        ...
        this.ipcListeners.push(
            window.arenaCoach.auth.onAuthSuccess((data) => {
        ...
        this.ipcListeners.push(
            window.arenaCoach.auth.onAuthError((error) => {
                console.error('Authentication error:', error);
                this.updateAuthState(false);
                NotificationManager.show(`Authentication failed: ${error}`, 'error');
            })
        );
    ...
    async initializeAuthState() {
        try {
            this.isAuthenticated = await window.arenaCoach.auth.isAuthenticated();
            if (this.isAuthenticated) {
                this.currentUser = await window.arenaCoach.auth.getCurrentUser();
            }
            this.updateAuthState(this.isAuthenticated, this.currentUser);
        } catch (error) {
    ...
    async handleBattleNetLogin() {
        try {
            ...
            if (errorMessage.includes('timeout') || errorMessage.includes('canceled')) {
                NotificationManager.show('Login canceled or timed out. Please try again.', 'info');
            } else {
                NotificationManager.show(`Login failed: ${errorMessage}`, 'error');
            }
            
            // Always re-enable button and reset text on error
            if (this.loginBtn) this.loginBtn.disabled = false;
            if (this.loginBtnLabel) this.loginBtnLabel.textContent = 'Battle.net Login';
        }
    }
    ...
    async handleLogout() {
        try {
            await window.arenaCoach.auth.logout();
        } catch (error) {
            console.error('Logout failed:', error);
            NotificationManager.show(`Logout failed: ${error}`, 'error');
        }
    }


    handleHelp() {
        NotificationManager.show('Help not implemented yet', 'info');
    }

    handleOpenWebsite() {
        // This should be handled by the main process for security
        NotificationManager.show('Website opening not implemented yet', 'info');
    }
